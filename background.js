  ...
  try {
    ...
    if (tab.url && tab.url.includes('chatgpt.com')) {
      try {
        ...
        console.log('Content script injected into ChatGPT tab');
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url) {
    try {
      ...
      // Inject content script into ChatGPT tabs when they finish loading
      if (tab.url.includes('chatgpt.com')) {
        try {
          ...
          console.log('Content script injected into loaded ChatGPT tab:', tabId);
